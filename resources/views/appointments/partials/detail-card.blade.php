...
// Only render as a Bulk Batch Workspace if this is a bulk appointment AND we are NOT in History View AND the user is either Staff or the Creator/Maker of that bulk batch.
$isBulkBatchWorkspace = ($app->batch_id && !$isHistoryView && ($isStaff || (auth()->check() && auth()->id() === $app->user_id)));
                    ...
                    <div class="bulk-actions-container">
                        @if($subApp->status == 'released')
                            ...
                            @if($isCoordinatorOnly)
                                <button class="btn btn-sm btn-accent py-1 px-3 fw-bold uppercase" data-bs-toggle="modal" data-bs-target="#forwardModal{{ $subApp->id }}" style="font-size: 0.7rem;">
                                    <i class="bi bi-send me-1"></i> FORWARD RESULT
                                </button>
                            @else
                                <div class="d-flex gap-1.5 align-items-center">
                                    @if($isStaff)
                                        <button type="button" class="btn btn-sm btn-outline-accent py-1 px-2.5 fw-bold" onclick="promptAccess('{{ $subApp->id }}', 'hub', 'edit')" style="font-size: 0.7rem;">
                                            <i class="bi bi-shield-lock-fill me-1"></i>VIEW RESULTS HUB
                                        </button>
                                        <button class="btn btn-sm btn-outline-accent py-1 px-2.5" data-bs-toggle="modal" data-bs-target="#forwardModal{{ $subApp->id }}" title="Forward Results to Email" style="font-size: 0.7rem;">
                                    ...
                                    @else
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-accent py-1 px-2.5 small uppercase hover-dark-text" data-bs-toggle="modal" data-bs-target="#forwardModal{{ $subApp->id }}" title="Forward Results to Email" style="font-size: 0.7rem;">
...
@if($isStaff)
{{-- Staff Confirm Batch Payment Modal --}}
...
{{-- Staff Revoke Batch Payment Modal --}}
...
{{-- Staff Return Entire Batch Modal --}}
...
{{-- Staff Approve Entire Batch Modal --}}
...
{{-- Forward Result Modals for All Sub-Appointments in Batch --}}
@foreach($batchAppointments as $subApp)
    @if($subApp->status == 'released')
        @php
...
<div id="details-{{ $app->id }}" class="appointment-detail-pane card border-secondary bg-card p-4 d-none animate-page">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2 text-start">
            <div>
                ...
                <div class="text-secondary smaller fw-bold uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px;">
                    ...
                    REF: #{{ $app->id }}
                    @if($app->batch_id)
                        <span class="mx-2">|</span> BATCH: #{{ $app->batch_id }}
                    @endif
                </div>
            </div>
            <div class="text-end">
    ...
    <div class="row g-4 text-start">
        ...
        <div class="col-md-7 border-end border-secondary border-opacity-25">
            ...
            {{-- Released Clinical Results Links (Patient View) --}}
                        ...
                        @foreach($app->result->included_reports ?? ['lab'] as $type)
                            ...
                            <div class="d-flex justify-content-between align-items-center p-2.5 border border-secondary border-opacity-10 rounded" style="background-color: rgba(0,0,0,0.015);">
                                ...
                                <div class="d-flex gap-1.5">
                                    ...
                                    <a href="{{ route('appointments.result.access', [$app->id, $type, 'download']) }}" class="btn btn-sm btn-accent py-1 px-2.5 small uppercase fw-bold" style="font-size: 0.7rem; color: #1c232d !important;">
                                        <i class="bi bi-download"></i> DOWNLOAD
                                    </a>
                                </div>
                            </div>
                            ...
                            @if($custom->status === 'verified')
                                <div class="d-flex justify-content-between align-items-center p-2.5 border border-secondary border-opacity-10 rounded" style="background-color: rgba(0,0,0,0.015);">
                                    ...
                                    <div class="d-flex gap-1.5">
                                        ...
                                        <a href="{{ route('appointments.result.access', [$app->id, 'custom_' . $custom->id, 'download']) }}" class="btn btn-sm btn-accent py-1 px-2.5 small uppercase fw-bold" style="font-size: 0.7rem; color: #1c232d !important;">
                                            <i class="bi bi-download"></i> DOWNLOAD
                                        </a>
                                    </div>
                                </div>
