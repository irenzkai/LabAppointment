...
<input type="text" name="patient_suffix" id="patient_suffix" list="suffix_options" class="form-control uppercase" value="{{ old('patient_suffix', $appointment->patient_suffix) }}" placeholder="e.g. JR">
...
{{-- 2. Address Card (PSGC Cascading Dropdowns) --}}
<div class="card p-4 border-secondary bg-card shadow-sm h-100">
<h5 class="text-accent fw-bold uppercase mb-3 small border-bottom pb-2" style="border-color: var(--border-color) !important;">
...
<select id="addr_province" name="patient_province" class="form-select" onchange="fetchCities(this.value)" required>
...
<select id="addr_city" name="patient_city" class="form-select" onchange="fetchBarangays(this.value)" disabled required>
...
<select id="addr_brgy" name="patient_barangay" class="form-select" disabled required>
...
<input type="text" name="patient_street" id="patient_street" class="form-control uppercase" value="{{ old('patient_street', $appointment->patient_street) }}" required>
...
<label class="form-check-label text-main small d-flex justify-content-between cursor-pointer w-100 align-items-center" for="svc_{{ $s->id }}">
...
@push('scripts')
<script>
const savedProvince = @json(old('patient_province', $appointment->patient_province));
const savedCity = @json(old('patient_city', $appointment->patient_city));
const savedBarangay = @json(old('patient_barangay', $appointment->patient_barangay));
...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Recover draft data if available
    loadDraftData();

    onServiceCheckChange();
    initPaymentState();
    await initAddressCascade();

    const initialDate = document.getElementById('appointment_date').value;
    if (initialDate) {
        await fetchTimeSlots(initialDate);
    }
    ...
    document.getElementById('resubmitForm').addEventListener('submit', validateResubmitForm);
});
...
/* -------------------------------------------------------------------------- */
/* INSTANT IN-PAGE "RESET EDITS" CONTROLLER (NO CONFIRMATION POPUP)          */
/* -------------------------------------------------------------------------- */
...
function updateTotal() {
    ...
    document.getElementById('total_bill_display').innerText = '₱' + total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
...
function handlePaymentMethodChange(radio) {
    ...
    if (targetMethod === 'Cash' && lastMethod === 'Cashless' && hasActiveReceipt()) {
        ...
        document.getElementById('pay_cashless').checked = true;
        return;
    }
...
function findMatchingOption(selectEl, textOrCode) {
    if (!selectEl || !textOrCode) return null;
    const target = textOrCode.toString().trim().toUpperCase();
    return Array.from(selectEl.options).find(opt => {
        if (opt.value.toString().trim().toUpperCase() === target) return true;
        if (opt.text.toString().trim().toUpperCase() === target) return true;
        const normOpt = opt.text.toUpperCase().replace(/\b(CITY|PROVINCE|MUNICIPALITY) OF\b/g, '').replace(/[^A-Z0-9]/g, '');
        const normTarget = target.replace(/\b(CITY|PROVINCE|MUNICIPALITY) OF\b/g, '').replace(/[^A-Z0-9]/g, '');
        return normOpt === normTarget && normOpt.length > 0;
    });
}
...
async function initAddressCascade() {
    ...
    try {
        let res = await fetch(`${psgcBase}/provinces.json`);
        if (!res.ok) res = await fetch(`${psgcBase}/provinces/`);
        const data = await res.json();
        
        provSel.innerHTML = '<option value="">Select Province</option>';
        data.sort((a,b) => a.name.localeCompare(b.name)).forEach(p => {
            provSel.innerHTML += `<option value="${p.name}" data-code="${p.code}">${p.name}</option>`;
        });

        if (savedProvince) {
            const matchOpt = findMatchingOption(provSel, savedProvince);
            if (matchOpt) {
                provSel.value = matchOpt.value;
                const provCode = matchOpt.dataset.code;
                if (provCode) await fetchCities(provCode);
            }
        }
    } catch (e) {
        console.error("PSGC Province Error", e);
    }
}
...
async function fetchCities(provCode) {
    ...
    try {
        let res = await fetch(`${psgcBase}/provinces/${provCode}/cities-municipalities.json`);
        if (!res.ok) res = await fetch(`${psgcBase}/provinces/${provCode}/cities-municipalities/`);
        const data = await res.json();
        ...
        if (savedCity) {
            const matchOpt = findMatchingOption(citySel, savedCity);
            if (matchOpt) {
                citySel.value = matchOpt.value;
                const cityCode = matchOpt.dataset.code;
                if (cityCode) await fetchBarangays(cityCode);
            }
        }
    } catch (e) {
        console.error("PSGC City Error", e);
    }
}

async function fetchBarangays(cityCode) {
    const brgySel = document.getElementById('addr_brgy');
    if (!cityCode || !brgySel) return;

    brgySel.disabled = true;
    brgySel.innerHTML = '<option value="">Loading Barangays...</option>';

    try {
        let res = await fetch(`${psgcBase}/cities-municipalities/${cityCode}/barangays.json`);
        if (!res.ok) res = await fetch(`${psgcBase}/cities-municipalities/${cityCode}/barangays/`);
        const data = await res.json();
        ...
        if (savedBarangay) {
            const matchOpt = findMatchingOption(brgySel, savedBarangay);
            if (matchOpt) brgySel.value = matchOpt.value;
        }
    } catch (e) {
        console.error("PSGC Barangay Error", e);
    }
}
...
async function fetchTimeSlots(dateStr) {
    ...
    try {
        ...
        while (start < end) {
            ...
            // Strictly exclude lunch, full, or past time slots
...
/* -------------------------------------------------------------------------- */
/* REAL-TIME BIRTHDATE VALIDATION CHECKER (Personal 18+ vs Dependent <18)     */
/* -------------------------------------------------------------------------- */
...
function validateResubmitForm(e) {
    ...
    // Standardize PSGC select option names to literal values before submission
    ['addr_province', 'addr_city', 'addr_brgy'].forEach(id => {
        const sel = document.getElementById(id);
        if (sel && sel.selectedIndex >= 0) {
            const opt = sel.options[sel.selectedIndex];
            if (opt && opt.text && !opt.text.includes('Select')) {
                opt.value = opt.text;
            }
        }
    });
