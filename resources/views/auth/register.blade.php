            ...
            <div class="row g-0 align-items-stretch">
                ...
                <div class="col-lg-5 d-none d-lg-flex flex-column justify-content-between p-5 bg-brand-dark position-relative" style="min-height: 600px;">
                    ...
                    <div class="position-relative" style="z-index: 3;">
                        ...
                        <p class="text-white-50 fs-5 mb-0" style="line-height: 1.6;">Follow our secure, multi-step registration flow to set up your personal clinical profile and gain immediate access to your lifetime diagnostic history.</p>
                    </div>
                ...
                <div class="col-lg-7 d-flex flex-column justify-content-center p-4 p-md-5 bg-card">
                    <div class="w-100" style="max-width: 480px; margin: 0 auto;">
                        ...
                        {{-- Promoted Dependent / Shadow Account Pre-Fill Banner --}}
                        @if(isset($promotedDependent))
                        <div class="alert alert-clinical border-accent bg-accent bg-opacity-10 text-main p-3 mb-4 rounded-3 d-flex align-items-center text-start">
                            <i class="bi bi-person-check-fill text-accent me-2.5 fs-4"></i>
                            <div>
                                <div class="fw-bold uppercase small text-accent">
                                    {{ !empty($promotedDependent->is_shadow) ? 'Activating Profile' : 'Promoting Dependent Profile' }}
                                </div>
                                <div class="small">
                                    Pre-filling details for <strong>{{ strtoupper($promotedDependent->first_name . ' ' . $promotedDependent->last_name) }}</strong>. Historical medical records and clinical findings will be automatically linked upon account registration.
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <form id="multiStepForm" method="POST" action="{{ route('register') }}" novalidate>
                            @csrf
                            @if(isset($promotedDependent))
                                @if(!empty($promotedDependent->shadow_appointment_id))
                                    <input type="hidden" name="shadow_appointment_id" value="{{ $promotedDependent->shadow_appointment_id }}">
                                @elseif(!empty($promotedDependent->id))
                            ...
                            <div class="reg-section" id="section-1">
                                <div class="row g-3 text-start">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="middle_name" id="middle_name" class="form-control uppercase" placeholder="Middle Name" value="{{ old('middle_name', (isset($promotedDependent) && $promotedDependent->middle_name === 'N/A') ? '' : ($promotedDependent->middle_name ?? '')) }}" maxlength="60">
                                    ...
                                    <div class="col-md-6 col-12">
                                        ...
                                        <input type="date" name="birthdate" class="form-control" value="{{ old('birthdate', isset($promotedDependent) && $promotedDependent->birthdate ? (\Carbon\Carbon::parse($promotedDependent->birthdate)->format('Y-m-d')) : '') }}" required max="{{ now()->subYears(18)->format('Y-m-d') }}">
                                    ...
                                    <div class="col-md-6 col-12">
                                        ...
                                        <select name="sex" class="form-select" required>
                                            <option value="Male" {{ strcasecmp(old('sex', $promotedDependent->sex ?? ''), 'Male') === 0 ? 'selected' : '' }}>Male</option>
                                            <option value="Female" {{ strcasecmp(old('sex', $promotedDependent->sex ?? ''), 'Female') === 0 ? 'selected' : '' }}>Female</option>
                                        </select>
                                    </div>
                                </div>
...
function saveDraft() {
    if (isPromotedAccount) return; // Never overwrite shadow/promoted parameters with stale client drafts
...
async function initializeAddress() {
    if (isPromotedAccount) {
        safeStorage.removeItem('register_draft');
        safeStorage.removeItem('register_step');
    }

    const draft = isPromotedAccount ? {} : JSON.parse(safeStorage.getItem('register_draft') || '{}');
    ...
    if (provinceVal) {
        ...
        if (provSel) {
            let provOpt = Array.from(provSel.options).find(opt => opt.text.toUpperCase().trim() === provinceVal.toUpperCase().trim() || opt.value === provinceVal);
            if (provOpt) {
                ...
                if (cityVal) {
                    const citySel = document.getElementById('addr_city');
                    if (citySel) {
                        let cityOpt = Array.from(citySel.options).find(opt => opt.text.toUpperCase().trim() === cityVal.toUpperCase().trim() || opt.value === cityVal);
                        if (cityOpt) {
                            ...
                            if (barangayVal) {
                                const brgySel = document.getElementById('addr_brgy');
                                if (brgySel) {
                                    let brgyOpt = Array.from(brgySel.options).find(opt => opt.text.toUpperCase().trim() === barangayVal.toUpperCase().trim() || opt.value === barangayVal);
                                    if (brgyOpt) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (phoneHiddenEl) {
        let rawPhone = phoneHiddenEl.value.trim().replace(/[^0-9]/g, '');
        if (rawPhone.startsWith('639')) rawPhone = '09' + rawPhone.substring(3);
        if (rawPhone.startsWith('9')) rawPhone = '09' + rawPhone;
        
        if (rawPhone && rawPhone.startsWith('09') && rawPhone.length === 11) {
            const displayInput = document.getElementById('phone_display');
            if (displayInput) {
                displayInput.value = rawPhone.substring(2);
                phoneHiddenEl.value = rawPhone;
            }
        }
    }

    const savedStep = isPromotedAccount ? 1 : safeStorage.getItem('register_step');
    if (savedStep) {
