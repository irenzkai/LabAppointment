...
@section('content')
<div class="container text-start animate-page py-4" id="edit-dependent-root">
            ...
            <div class="card p-4 p-md-5 border-secondary bg-card shadow-lg">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 border-bottom border-secondary border-opacity-25 pb-3">
                    <div>
                        ...
                        <p class="text-secondary mb-0 small">Update identity, demographics, or address details for {{ $dependent->name }}.</p>
                    </div>
                ...
                @php
                $isOver18 = $dependent->birthdate->age >= 18;
                $hasNoMiddleName = empty($dependent->middle_name) || $dependent->middle_name === 'N/A';
                $promoUrl = route('register', ['promote' => \Illuminate\Support\Facades\Crypt::encryptString($dependent->id)]);
                @endphp

                @if($isOver18)
                <div class="alert alert-warning border-warning p-4 mb-4 rounded-3 text-start shadow-sm" style="background-color: rgba(255, 193, 7, 0.05) !important;">
                    <div class="d-flex align-items-center mb-2">
                        <i class="bi bi-exclamation-triangle-fill fs-4 text-warning me-3"></i>
                        <div>
                            <h5 class="fw-bold uppercase mb-0 text-warning">Minor Status Expired (18+ Years Old)</h5>
                            <small class="text-secondary">This family member has reached 18 years of age.</small>
                        </div>
                    </div>
                    <p class="small text-main mb-3">
                        Under clinical regulations, dependent profiles are for minors under 18 years of age only. Profile editing is disabled. Please promote this account so they can manage their own independent profile.
                    </p>
                    <button type="button" class="btn btn-accent fw-bold uppercase btn-sm py-2 px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#promoteModal{{ $dependent->id }}">
                        <i class="bi bi-arrow-up-circle me-1.5"></i> Promote Account Now
                    </button>
                </div>
                @endif
                ...
                {{-- Edit Dependent Form --}}
                <form action="{{ route('dependents.update', $dependent->id) }}" method="POST" id="editDependentForm" onsubmit="return validateEditDependentForm(event)">
                    @csrf
                    @method('PUT')
                    <fieldset {{ $isOver18 ? 'disabled' : '' }}>
                        {{-- 1. Personal Identity --}}
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="first_name" id="in_first_name" class="form-control uppercase @error('first_name') is-invalid @enderror" value="{{ old('first_name', $dependent->first_name) }}" oninput="this.value = this.value.replace(/[^a-zA-ZñÑ \s.\'-]/g, '')" required>
                            ...
                            <div class="col-md-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="edit_dep_no_mn" onclick="toggleEditDepMN(this)" {{ $hasNoMiddleName ? 'checked' : '' }}>
                                        <label class="smaller text-secondary" style="font-size: 0.65rem;" for="edit_dep_no_mn">None</label>
                                    </div>
                                </div>
                                <input type="text" name="middle_name" id="edit_dep_middle_name" class="form-control uppercase {{ $hasNoMiddleName ? 'opacity-50' : '' }} @error('middle_name') is-invalid @enderror" value="{{ old('middle_name', $dependent->middle_name ?? 'N/A') }}" oninput="this.value = this.value.replace(/[^a-zA-ZñÑ \s.\'-]/g, '')" {{ $hasNoMiddleName ? 'readonly' : '' }}>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="last_name" id="in_last_name" class="form-control uppercase @error('last_name') is-invalid @enderror" value="{{ old('last_name', $dependent->last_name) }}" oninput="this.value = this.value.replace(/[^a-zA-ZñÑ \s.\'-]/g, '')" required>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="suffix" id="edit_dep_suffix" list="suffix_options" class="form-control uppercase @error('suffix') is-invalid @enderror" value="{{ old('suffix', $dependent->suffix) }}" placeholder="e.g. JR" maxlength="10">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-4">
                                ...
                                <input type="date" name="birthdate" id="in_birthdate" class="form-control @error('birthdate') is-invalid @enderror" value="{{ old('birthdate', $dependent->birthdate->format('Y-m-d')) }}" required min="{{ now()->subYears(18)->addDay()->format('Y-m-d') }}" max="{{ now()->format('Y-m-d') }}" onchange="validateBirthdateInput()">
                                <div class="invalid-feedback d-none" id="err_birthdate"></div>
                                @error('birthdate')
                                <div class="text-danger small mt-1 fw-bold">{{ $message }}</div>
                                @enderror
                                <small class="text-muted smaller mt-1 d-block"><i class="bi bi-info-circle me-1"></i>Dependents must be under 18 years old. If this family member is 18 or older, please promote this account so they can register their own profile.</small>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="sex" id="in_sex" class="form-select @error('sex') is-invalid @enderror" required>
                                    <option value="Male" {{ old('sex', $dependent->sex) == 'Male' ? 'selected' : '' }}>Male</option>
                                    <option value="Female" {{ old('sex', $dependent->sex) == 'Female' ? 'selected' : '' }}>Female</option>
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="relationship" id="in_relationship" class="form-select @error('relationship') is-invalid @enderror" required>
                                    <option value="Son" {{ strtoupper(old('relationship', $dependent->relationship)) === 'SON' ? 'selected' : '' }}>Son</option>
                                    <option value="Daughter" {{ strtoupper(old('relationship', $dependent->relationship)) === 'DAUGHTER' ? 'selected' : '' }}>Daughter</option>
                                </select>
                        ...
                        <div class="border-top border-secondary border-opacity-25 pt-3 mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h6 class="text-accent small fw-bold uppercase mb-0">3. Home Address (PSGC API)</h6>
                                <button type="button" class="btn btn-sm btn-outline-accent py-1 px-3 fw-bold" onclick="fetchParentAddress()">
                                    <i class="bi bi-geo-alt-fill me-1.5"></i>Use Parent's Address
                                </button>
                            </div>
                            <div id="manual_edit_dep_address_wrapper" class="row g-3">
                                <div class="col-md-6">
                                    <label class="small text-secondary fw-bold mb-1 uppercase">Province</label>
                                    <select id="edit_dep_province" name="province" class="form-select @error('province') is-invalid @enderror" onchange="fetchEditDepCities(this.value, '', '')" required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="edit_dep_city" name="city" class="form-select @error('city') is-invalid @enderror" onchange="fetchEditDepBarangays(this.value, '')" disabled required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="edit_dep_barangay" name="barangay" class="form-select @error('barangay') is-invalid @enderror" disabled required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="edit_dep_street" name="street" class="form-control uppercase @error('street') is-invalid @enderror" value="{{ old('street', $dependent->street) }}" required>
...
{{-- INDEPENDENT ACCOUNT PROMOTION MODAL --}}
<div class="modal fade" id="promoteModal{{ $dependent->id }}" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 480px;">
        <div class="modal-content border-secondary bg-card text-start p-4" style="background-color: var(--bg-card); border: 1.5px solid var(--border-color); color: var(--text-main);">
            <div class="modal-header border-secondary bg-secondary bg-opacity-10 py-3">
                <h5 class="modal-title text-accent fw-bold small uppercase mb-0">
                    <i class="bi bi-arrow-up-circle me-2"></i>Promote to Independent Account
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <p class="small text-muted mb-3">
                    Promoting <strong>{{ strtoupper($dependent->name) }}</strong> allows them to create their own independent profile. All historic appointment results, medical files, and diagnostic history will be safely transferred to their new account, and this dependent profile will be removed from your family list once registered.
                </p>
                <div class="alert alert-clinical border-secondary bg-dark bg-opacity-25 p-3 rounded mb-4" style="font-size: 0.75rem;">
                    ...
                    <strong>Clinical History Retention:</strong> Past records will automatically belong to the new independent user account.
                </div>

                {{-- Shareable Link Input --}}
                <div class="mb-4">
                    <label class="smaller text-secondary fw-bold mb-1 uppercase d-block" style="font-size: 0.75rem; letter-spacing: 0.5px;">Shareable Registration Link</label>
                    <div class="input-group">
                        <input type="text" class="form-control form-control-sm" value="{{ $promoUrl }}" id="link-{{ $dependent->id }}" style="background-color: var(--bg-main) !important; color: var(--text-main) !important; border-color: var(--border-color) !important;" readonly>
                        <button class="btn btn-outline-accent btn-sm fw-bold" onclick="copyPromoLink('{{ $dependent->id }}')">Copy Link</button>
                    </div>
                    <div id="copy-success-{{ $dependent->id }}" class="text-success small mt-1 d-none fw-bold" style="font-size: 0.75rem;">
                        <i class="bi bi-check-circle-fill me-1"></i> Link copied to clipboard!
                    </div>
                </div>

                {{-- Direct Logout & Register Button --}}
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button type="submit" class="btn-custom btn-accent w-100 py-3 fw-bold uppercase shadow-sm">
                        <i class="bi bi-box-arrow-right me-1.5"></i> Logout & Register Account Now
                    </button>
                </form>
            </div>
        </div>
...
#edit-dependent-root .form-control:focus,
#edit-dependent-root .form-select:focus {
    border-color: var(--brand-accent) !important;
    box-shadow: 0 0 0 3px rgba(25, 211, 140, 0.15) !important;
}
#edit-dependent-root .is-invalid {
...
function toggleEditDepMN(checkbox) {
    const input = document.getElementById('edit_dep_middle_name');
    if (input) {
        if (checkbox.checked) {
            input.value = "N/A";
            input.readOnly = true;
...
function validateEditDependentForm(e) {
    let isValid = true;
    let firstInvalidInput = null;

    document.querySelectorAll('#editDependentForm .is-invalid').forEach(el => el.classList.remove('is-invalid'));
    document.querySelectorAll('#editDependentForm .invalid-feedback').forEach(el => {
        el.innerText = '';
    ...
    const mNameInput = document.getElementById('edit_dep_middle_name');
    ...
    const suffixInput = document.getElementById('edit_dep_suffix');
    if (suffixInput && suffixInput.value.trim()) {
        const suffixRegex = /^[a-zA-Z\s.]+$/;
        if (!suffixRegex.test(suffixInput.value.trim())) markInvalid(suffixInput, 'err_suffix', 'Suffix may only contain letters, numbers, spaces, and periods.');
    ...
    const relSel = document.getElementById('in_relationship');
    if (!relSel || !relSel.value) markInvalid(relSel, 'err_relationship', 'Please select a relationship.');
    ...
    const streetInput = document.getElementById('edit_dep_street');
    ...
    compileEditDepAddress();
    return true;
}

async function fetchEditDepCities(provCode, savedCity, savedBrgy) {
    const citySel = document.getElementById('edit_dep_city');
    const brgySel = document.getElementById('edit_dep_barangay');
    ...
    try {
        ...
        if (savedCity) {
            const selectedCityOpt = Array.from(citySel.options).find(opt => 
                opt.text.toUpperCase().trim() === savedCity.toUpperCase().trim() || opt.value === savedCity.trim()
            );
            if (selectedCityOpt) {
                citySel.value = selectedCityOpt.value;
                await fetchEditDepBarangays(selectedCityOpt.value, savedBrgy);
            }
        }
    } catch (e) {
...
async function fetchEditDepBarangays(cityCode, savedBrgy) {
    const brgySel = document.getElementById('edit_dep_barangay');
    if (!brgySel) return;
    brgySel.disabled = true;
    ...
    try {
        ...
        if (savedBrgy) {
            const selectedBrgyOpt = Array.from(brgySel.options).find(opt => 
                opt.text.toUpperCase().trim() === savedBrgy.toUpperCase().trim() || opt.value === savedBrgy.trim()
            );
            if (selectedBrgyOpt) {
...
async function initEditDepAddress(savedProv, savedCity, savedBrgy, savedStreet) {
    const provSel = document.getElementById('edit_dep_province');
    const citySel = document.getElementById('edit_dep_city');
    const brgySel = document.getElementById('edit_dep_barangay');
    const streetInput = document.getElementById('edit_dep_street');
    ...
    try {
        ...
        if (savedProv) {
            let provOpt = Array.from(provSel.options).find(opt => 
                opt.text.toUpperCase().trim() === savedProv.toUpperCase().trim() || opt.value === savedProv.trim()
            );
            if (provOpt) {
                provSel.value = provOpt.value;
                let cityRes = await fetch(`https://psgc.gitlab.io/api/provinces/${provOpt.value}/cities-municipalities/`);
...
function compileEditDepAddress() {
...
document.addEventListener('DOMContentLoaded', async () => {
    const savedProv = "{{ $dependent->province }}";
    const savedCity = "{{ $dependent->city }}";
    const savedBrgy = "{{ $dependent->barangay }}";
    const savedStreet = "{{ $dependent->street }}";

    await initEditDepAddress(savedProv, savedCity, savedBrgy, savedStreet);

    document.querySelectorAll('#editDependentForm input, #editDependentForm select').forEach(input => {
